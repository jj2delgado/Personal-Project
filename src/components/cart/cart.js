...
import {getCart, pay} from '../../ducks/cartReducer'
import CartProduct from '../cartProduct/CartProduct'
import StripeCheckout from 'react-stripe-checkout'
import axios from 'axios'
// require('dotenv').config({path: __dirname + "/../../../.env"})
// const {PUBLIC_STRIPE} = process.env
...
class Cart extends Component{
    ...
    //     console.log(token)
    //     // token.card = void 0
    //     this.props.pay(this.props.user.user.id)
    // }
    onToken = (token) => {
        console.log(token)
    ...
    render(){
        console.log(this.props)
        console.log(process.env.PUBLIC_STRIPE)
        return(
            <div>
                <h1>YOUR CART</h1>
                <div>
...
export default connect(mapStateToProps, {getCart, pay})(Cart)